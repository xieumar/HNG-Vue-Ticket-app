<template>
  <div class="max-w-2xl mx-auto p-4 bg-white shadow-md rounded-lg">
    ...
    <div v-if="ticket">
      ...
      <p class="text-gray-600 text-sm">Status: <span :class="{'text-green-600': ticket.status === 'open', 'text-amber-600': ticket.status === 'in progress', 'text-gray-600': ticket.status === 'closed'}">{{ ticket.status }}</span></p>
